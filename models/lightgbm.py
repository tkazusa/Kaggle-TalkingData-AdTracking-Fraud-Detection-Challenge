...
class LightGBM(Model):
    def train_and_predict(self, train, valid, categorical_features: List[str], target: str, params: dict) \
        ...
        weights = {i: round(np.sum(classdist) /
                            classdist[i]) for i in classdist.index}
        weight_list_train = [weights[i] for i in train[target].values]
        weight_list_valid = [weights[i] for i in valid[target].values]

        d_train = lgb.Dataset(train.drop(
            target, axis=1), label=train[target].values, weight=weight_list_train)
        d_valid = lgb.Dataset(valid.drop(
            target, axis=1), label=valid[target].values, weight=weight_list_valid)
